 ...
 *
 * @see
 *   <a href="http://en.wikipedia.org/wiki/Token_bucket">Token Bucket on Wikipedia</a>
 *
 * @param ratePerPeriod
 *   Allowed number of tokens which can be consumed per period.
...
case class TokenBucket(
  ...
  /**
   * Returns the current number of tokens in the bucket. If the bucket is empty then this method
   * will return 0.
   *
   * @return
  ...
  /**
   * Attempt to consume a specified number of tokens from the bucket. If the tokens were consumed
  ...
  /**
   * Refills the bucket with the specified number of tokens. If the bucket is currently full or near
   * capacity then fewer than {{{numTokens}}} may be added.
   *
   * @param numTokens
